...
class GestorPerfilesModel{
	...
	public function guardarPerfilModel($datosModel, $tabla){
        ...
        $obj = new Conexion();
		$stmt = $obj->conectar()->prepare("INSERT INTO $tabla (usuario, password, email, photo, rol) VALUES (:usuario, :password, :email, :photo, :rol)");
	...
	public function editarPerfilModel($datosModel, $tabla){
		...
		$obj = new Conexion();
		$stmt = $obj->conectar()->prepare("UPDATE $tabla SET usuario = :usuario, password = :password, email = :email, rol = :rol, photo = :photo WHERE id = :id");	
